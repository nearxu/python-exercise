...
def login(username, password, oncaptcha, sessiona, header):
    resp1 = sessiona.get(
    ...
    need_cap = json.loads(
        resp2.text)["show_captcha"]  # {"show_captcha":false} 表示不用验证码
    ...
    data = {
        ...
        "timestamp": timestamp,
        "source": source,
        "signature": get_signature(grantType, clientId, source,
                                   timestamp),  # 获取签名
        "username": username,
        "password": password,
        "lang": "cn",
    ...
    print("-" * 50)
    resp = sessiona.post(
        'https://www.zhihu.com/api/v3/oauth/sign_in', data,
        headers=headers).content
    return resp
...
if __name__ == '__main__':
    ...
    login('**' * 10, '*' * 10, get_captcha, sessiona, headers)  # 用户名密码换自己的就好了
    resp = sessiona.get(
        'https://www.zhihu.com/inbox', headers=headers)  # 登录进去了，可以看私信了
