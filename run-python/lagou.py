...
# 这是定义的需要保存的字段，（导入dingdian项目中，items文件中的DingdianItem类）
...
class Myspider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        urllist = BeautifulSoup(response.text, 'lxml').find_all(
            'div', class_='p_top')
    ...
    def get_shuju(self, response):
        ...
        item['yaoqiu'] = BeautifulSoup(response.text, 'lxml').find(
            'dd', class_='job_bt').get_text()
        self.zanting()
        return item
